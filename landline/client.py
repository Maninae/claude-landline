...
This file stays as a thin facade so the daemon-wide imports (`from landline.client
import send_response, send_html, …`) and the test suite's patch targets
(`landline.client._send_chunk`, `landline.client.send_response`, etc.) keep
resolving without churn.
"""

# Logging — re-exported so callers / tests can patch `landline.client.log` if
...
from landline.telegram_fmt import md_to_telegram_html

# Tag-aware HTML chunker internals + `send_html`.
from landline.html_chunker import (
...
# markdown chunker.
from landline.telegram_transport import (
